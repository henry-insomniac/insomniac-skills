...
npm 包提供 `init-agent-docs` 命令。该命令内部调用 Python 标准库脚本，因此用户机器需要安装 `python3`。

The npm package provides the `init-agent-docs` command. It runs a Python standard-library script internally, so `python3` must be available on the user's machine.
...
```bash
npx -p insomniac-skills init-agent-docs [options]
```
...
```bash
npx -p insomniac-skills init-agent-docs --help
```
...
```bash
npx -p insomniac-skills init-agent-docs --list-skills
```
...
| `--project-name PROJECT_NAME` | 目标目录名 / target directory name | 项目名称，会写入生成文档。 / Project name written into generated docs. |
| `--description DESCRIPTION` | `这是一个需要维护长期 Agent 上下文的项目。` | 项目描述，会写入生成文档。 / Project description written into generated docs. |
...
| `--with-agent-ops` | 关闭 / off | 生成 `docs/agents/*`、`docs/adr/README.md`，并按领域布局生成 `CONTEXT.md` 或 `CONTEXT-MAP.md`。 / Generate `docs/agents/*`, `docs/adr/README.md`, and `CONTEXT.md` or `CONTEXT-MAP.md` based on domain layout. |
| `--with-design` | 关闭 / off | 生成根目录 `DESIGN.md`，并在 Agent 入口中加入 UI 任务读取提示。 / Generate root `DESIGN.md` and add UI-task guidance to Agent entry docs. |
| `--issue-tracker auto\|github\|gitlab\|local\|manual` | `manual` | 配置 Agent 操作规则层中的 issue tracker 文档。只有配合 `--with-agent-ops` 才会写入对应文件。 / Configure issue tracker docs for Agent operating rules. Only writes files when used with `--with-agent-ops`. |
